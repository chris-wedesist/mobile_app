...
const testLocations = [
  { name: 'New York City', lat: 40.7128, lng: -74.0060, state: 'NY' },
...
export const testRealAPIIntegration = async (): Promise<void> => {
  ...
  try {
    ...
    const availableStateBars = stateBarAPIs.filter(state => hasStateBarAPI(state));
    console.log(`✅ State Bar APIs available: ${availableStateBars.join(', ')}`);
    
    const legalAidOrgs = ['CA_CRLA', 'TX_TLSC', 'FL_FLS', 'NY_LANYC', 'IL_CLA'];
    const availableLegalAid = legalAidOrgs.filter(org => hasLegalAidOrganization(org));
    console.log(`✅ Legal Aid Organizations available: ${availableLegalAid.join(', ')}`);
    
    const civilRightsOrgs = ['ACLU', 'NLG', 'SPLC', 'LAMBDA_LEGAL'];
    const availableCivilRights = civilRightsOrgs.filter(org => hasCivilRightsOrganization(org));
    console.log(`✅ Civil Rights Organizations available: ${availableCivilRights.join(', ')}`);
  ...
  for (const location of testLocations.slice(0, 3)) { // Test first 3 locations
    ...
    try {
      ...
      if (hasStateBarAPI(location.state)) {
        ...
        console.log(`  📊 State Bar: ${stateBarAttorneys.length} attorneys found`);
      }
    ...
    try {
      ...
      if (attorneys.length > 0) {
        ...
        const requiredFields = [
          'id', 'name', 'specialization', 'lat', 'lng', 'verified', 'source', 'lastVerified'
        ];
        
        const missingFields = requiredFields.filter(field => !(field in firstAttorney));
        if (missingFields.length === 0) {
          console.log(`  ✅ Attorney data structure verified`);
          console.log(`  📋 Sample attorney: ${firstAttorney.name} (${firstAttorney.specialization})`);
        ...
        console.log(`  ⚠️ No attorneys found (this is acceptable for real API testing)`);
      }
  ...
  try {
    ...
    const fakeSourceAttorneys = attorneys.filter(attorney => 
    ...
    if (unverifiedAttorneys.length === 0 && fakeSourceAttorneys.length === 0) {
      ...
      if (attorneys.length > 0) {
        console.log(`📋 Sources: ${[...new Set(attorneys.map(a => a.source))].join(', ')}`);
      }
      passedTests++;
    } else {
