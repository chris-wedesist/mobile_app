...
export const useExtendedStore = create<AppExtendedState>((set, get) => ({
  ...
  setLoading: (key: string, loading: boolean) =>
    set((state) => ({
      ...
      isLoading: Object.values({ ...state.loadingStates, [key]: loading }).some(
        Boolean
      ),
    })),
  ...
  setError: (key: string, error: string | null) =>
    set((state) => ({
      ...
      error:
        error ||
        Object.values({ ...state.errors, [key]: error }).find(Boolean) ||
        null,
    })),
...
export class StateManager {
  ...
  static async fetchWithState<T>(
    ...
    try {
      ...
      const data = await performanceOptimizer.fetchWithCache(
        key,
        fetchFunction,
        {
