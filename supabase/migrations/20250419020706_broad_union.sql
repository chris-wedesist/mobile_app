/*
  ...
  1. New Fields
    - Add remaining badge fields to user_settings table
    - Add badge event tracking table
  ...
  3. Functions
    - Add helper functions for badge management
*/

-- Add remaining badge fields to user_settings
DO $$ 
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_settings' 
    AND column_name = 'badge_emergency_sentinel'
  ) THEN
    ALTER TABLE user_settings 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_settings' 
    AND column_name = 'badge_evidence_guardian'
  ) THEN
    ALTER TABLE user_settings 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_settings' 
    AND column_name = 'badge_community_defender'
  ) THEN
    ALTER TABLE user_settings 
...
-- Create badge requirements table
...
-- Add RLS policies for badge_events
...
-- Add RLS policies for badge_requirements
...
-- Insert initial badge requirements
INSERT INTO badge_requirements (badge_name, requirement_type, requirement_value, description) VALUES
