...
export default function LegalHelpScreen() {
  ...
  const getFilteredAttorneys = () => {
    ...
    if (state.filters.rating.enabled) {
      filtered = filtered.filter(
        (attorney) => attorney.rating >= state.filters.rating.minRating
      );
    }
    ...
    if (state.filters.reviews.enabled) {
      filtered = filtered.filter(
        (attorney) =>
          attorney.userRatingsTotal >= state.filters.reviews.minReviews
      );
    }
    ...
    if (state.filters.openNow) {
      filtered = filtered.filter(
        (attorney) => attorney.openingHours?.openNow === true
      );
    }

    if (state.filters.verified) {
      filtered = filtered.filter(
        (attorney) => attorney.businessStatus === 'OPERATIONAL'
      );
    }

    if (state.filters.selectedTags.length > 0) {
      filtered = filtered.filter((attorney) => {
        return attorney.tags?.some((tag) =>
      ...
      <View style={styles.radiusContainer}>
        ...
        <View style={styles.radiusDisplay}>
          ...
          <Text style={styles.radiusCountText}>
            {state.attorneys.length} attorney
            {state.attorneys.length !== 1 ? 's' : ''} found
          </Text>
        </View>
          ...
          <View style={styles.radiusControlContainer}>
            <View style={styles.radiusButtonsContainer}>
              <TouchableOpacity
                ...
                onPress={() =>
                  updateState({ radius: Math.max(1, state.radius - 5) })
                }
              >
                <MaterialIcons
                  name="remove"
              ...
              <TouchableOpacity
                style={styles.radiusButton}
                onPress={() =>
                  updateState({ radius: Math.min(50, state.radius + 5) })
                }
              >
                <MaterialIcons
                  name="add"
                      ...
                      <TouchableOpacity
                          ...
                          styles.filterChip,
                          {
                            backgroundColor:
                              state.filters.selectedTags.includes(tag)
                                ? tagColor.background
                                : colors.surface,
                            borderColor: state.filters.selectedTags.includes(
                              tag
                            )
                              ? tagColor.border
                              : colors.border,
                        ...
                        onPress={() => {
                          const newSelectedTags =
                            state.filters.selectedTags.includes(tag)
                              ? state.filters.selectedTags.filter(
                                  (t) => t !== tag
                                )
