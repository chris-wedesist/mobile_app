...
const APP_LOCK_SALT = 'DESIST_APP_LOCK';
...
export class AppLockManager {
  ...
  private lockStateCallbacks: Array<(state: LockState) => void> = [];
    ...
    try {
      ...
      if (this.lockState.isLockedOut) {
        ...
        if (this.lockState.lockoutExpiresAt && now < this.lockState.lockoutExpiresAt) {
          const remainingTime = Math.ceil((this.lockState.lockoutExpiresAt.getTime() - now.getTime()) / SECONDS_TO_MS);
          return {
            success: false,
  ...
  private async authenticateWithPin(pin: string): Promise<{ success: boolean; error: string }> {
    try {
      ...
      const pinHash = await this.cryptoManager.generateSecureHash(`${pin}${APP_LOCK_SALT}`);
   ...
   */
  onLockStateChange(callback: (_state: LockState) => void): void {
   ...
   */
  removeLockStateCallback(callback: (_state: LockState) => void): void {
  ...
  private async storePinHash(_hash: string): Promise<void> {
...
export const onLockStateChange = (callback: (_state: LockState) => void) => 
